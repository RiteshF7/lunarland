...
class DroidrunAccessibilityService : AccessibilityService(), ConfigManager.ConfigChangeListener {
    ...
    // Public methods for SDK to call directly
     ...
     * @param overlayIndex The index shown in the overlay
     * @return true if click was successful, false otherwise
     */
    fun clickElementByIndex(overlayIndex: Int): Boolean {
        return try {
            val rootNode = rootInActiveWindow ?: return false
            
            // Get current elements to find the target
            val elements = getVisibleElementsInternal()
            val targetElement = elements.firstOrNull { it.overlayIndex == overlayIndex }
            
            if (targetElement == null) {
                Log.e(TAG, "Element with index $overlayIndex not found")
                rootNode.recycle()
                return false
            }
            
            // Find the corresponding AccessibilityNodeInfo in the current tree
            val targetNode = findNodeByProperties(rootNode, targetElement)
            
            if (targetNode == null) {
                Log.e(TAG, "Could not find AccessibilityNodeInfo for element with index $overlayIndex")
                rootNode.recycle()
                return false
            }
            ...
            if (!targetNode.isClickable) {
                Log.e(TAG, "Element with index $overlayIndex is not clickable")
                targetNode.recycle()
                rootNode.recycle()
                return false
            }
            ...
            if (success) {
                Log.d(TAG, "Successfully clicked element with index $overlayIndex")
            } else {
                Log.e(TAG, "Failed to perform click on element with index $overlayIndex")
            }
    ...
    /**
     * Helper method to find a node matching the element's properties
     */
    private fun findNodeByProperties(root: AccessibilityNodeInfo, target: ElementNode): AccessibilityNodeInfo? {
        val rect = Rect()
        root.getBoundsInScreen(rect)
        
        // Check if this node matches
        if (rect == target.rect && 
            root.className?.toString()?.substringAfterLast('.') == target.className &&
            (root.text?.toString() == target.text || 
             root.contentDescription?.toString() == target.text ||
             root.viewIdResourceName?.substringAfterLast('/') == target.text)) {
        ...
        for (i in 0 until root.childCount) {
            ...
            try {
                val found = findNodeByProperties(child, target)
                if (found != null) {
                    return found
                }
            } finally {
